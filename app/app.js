...
// ============================================================
// TEMP: Nav debug overlay — tune bottom nav live, then remove
// ============================================================
(function initNavDebug() {
  ...
  function readPx(v) { return parseFloat(v) || 0; }
  function render() {
    const cs = getComputedStyle(document.querySelector('nav'));
    const btn = document.querySelector('.nav-btn');
    const btnCs = btn ? getComputedStyle(btn) : null;
    const iconWrap = document.querySelector('.nav-btn .nav-icon-wrap');
    const safe = getComputedStyle(root).getPropertyValue('--safe-bottom').trim() || '0px';
    document.getElementById('nd-safe').textContent = safe;
    document.getElementById('nd-padb').textContent = readPx(cs.paddingBottom).toFixed(0) + 'px';
    document.getElementById('nd-padt').textContent = btnCs ? readPx(btnCs.paddingTop).toFixed(0) + 'px' : '—';
  ...
  if (localStorage.getItem(LS_HIDE) === '1') el.classList.add('hidden');
  apply();
  // Re-render on resize (orientation, keyboard, etc.)
